import { Client, registry, MissingWalletError } from 'belShare-client-ts'

import { Params } from "belShare-client-ts/belshare.eav/types"
import { User } from "belShare-client-ts/belshare.eav/types"
...
export default {
	...
	actions: {
		init({ dispatch, rootGetters }) {
			console.log('Vuex module: belshare.eav initialized!')
		...
		async QueryParams({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				...
				let value= (await client.BelshareEav.query.queryParams()).data
		...
		async QueryUser({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				const key = params ?? {};
				const client = initClient(rootGetters);
				let value= (await client.BelshareEav.query.queryUser( key.index)).data
		...
		async QueryUserAll({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				const key = params ?? {};
				const client = initClient(rootGetters);
				let value= (await client.BelshareEav.query.queryUserAll(query ?? undefined)).data
				
					
				while (all && (<any> value).pagination && (<any> value).pagination.next_key!=null) {
					let next_values=(await client.BelshareEav.query.queryUserAll({...query ?? {}, 'pagination.key':(<any> value).pagination.next_key} as any)).data
		...
		async sendMsgCreateUser({ rootGetters }, { value, fee = [], memo = '' }) {
			try {
				...
				const result = await client.BelshareEav.tx.sendMsgCreateUser({ value, fee: {amount: fee, gas: "200000"}, memo })
				return result
			} catch (e) {
		...
		async MsgCreateUser({ rootGetters }, { value }) {
			try {
				const client=initClient(rootGetters)
				const msg = await client.BelshareEav.tx.msgCreateUser({value})
				return msg
			} catch (e) {
				if (e == MissingWalletError) {
					throw new Error('TxClient:MsgCreateUser:Init Could not initialize signing client. Wallet is required.')
